...
with manifest as (select * from jsonb_to_recordset('[
 ...
 ]'::jsonb) as x(level int,num int,sort int,title text)), course_identity as(select id from public.courses where slug='music-business-artist-development'), counts as (
