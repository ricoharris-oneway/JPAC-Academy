-- JPAC Digital AI Creator 48-module rollout post-validation. READ ONLY.
...
with manifest as (select * from jsonb_to_recordset('[
 ...
 select count(*)::int n,(array_agg(id order by id::text))[1] id,(array_agg(slug order by id::text))[1] slug,(array_agg(title order by id::text))[1] title from public.courses where slug='digital-ai-creator'
), counts as (
 ...
 select (select count(*) from public.course_modules m join public.course_levels l on l.id=m.course_level_id where m.course_id=(select id from course_identity) and l.level_number=1 and m.level_module_number=12 and m.title='Level 1 Showcase: My First AI Mini Story' and m.review_notes like '%Approved level capstone%')::int beginner,
 (select count(*) from public.course_modules m join public.course_levels l on l.id=m.course_level_id where m.course_id=(select id from course_identity) and l.level_number=2 and m.level_module_number=12 and m.title='Level 2 Showcase: AI Commercial or Music Campaign' and m.review_notes like '%Approved level capstone%')::int intermediate,
 (select count(*) from public.course_modules m join public.course_levels l on l.id=m.course_level_id where m.course_id=(select id from course_identity) and l.level_number=3 and m.level_module_number=12 and m.title='Level 3 Showcase: AI Director Project' and m.review_notes like '%Approved level capstone%')::int advanced,
 (select count(*) from public.course_modules m join public.course_levels l on l.id=m.course_level_id where m.course_id=(select id from course_identity) and l.level_number=4 and m.level_module_number=12 and m.title='Master''s Magnum Opus: The JPAC AI Production' and m.review_notes like '%Approved level capstone%')::int master
), review_flags as (
 select count(*) filter(where review_notes like '%MEDIA NEEDS REVIEW%' and review_notes like '%NEEDS CATALOG REVIEW%' and review_notes like '%AI SAFETY REVIEW%' and review_notes like '%MINOR ACCESS REVIEW%' and review_notes like '%PRIVACY/CONSENT REVIEW%' and review_notes like '%LIKENESS/PERMISSION REVIEW%' and review_notes like '%COPYRIGHT/IP REVIEW%' and review_notes like '%DECEPTIVE MEDIA REVIEW%' and review_notes like '%DISCLOSURE REVIEW%' and review_notes like '%PLATFORM POLICY REVIEW%' and review_notes like '%SCOPE REVIEW%' and review_notes like '%JPAC-native private submission%')::int complete_flags
 ...
 select 'IDENTITY' section,'DIGITAL_AI_CREATOR_IDENTITY' code,case when n=1 and slug='digital-ai-creator' and title='Digital AI Creator' then 'PASS' else 'BLOCK' end result,format('rows=%s/1; slug=%s; title=%s',n,coalesce(slug,'NULL'),coalesce(title,'NULL')) details from course_identity
 ...
 union all select 'SAFETY','DIGITAL_AI_CREATOR_DEPENDENCIES',case when n=0 then 'PASS' else 'BLOCK' end,format('student/evidence dependencies=%s',n) from deps
...
union all select 'READINESS','OVERALL',case when n=0 then 'PASS' else 'BLOCK' end,case when n=0 then 'PASS: DIGITAL AI CREATOR FULL DRAFT ROLLOUT VALID; COMPARE PRESERVATION BASELINES' else 'BLOCK: REVIEW DIGITAL AI CREATOR ROLLOUT' end from blockers
