...
with manifest as (select * from jsonb_to_recordset('[
 ...
 select count(*)::int n,(array_agg(id order by id::text))[1] id from public.courses where slug='music-business'
), sdi as (
 ...
 select count(*)::int n from public.course_modules m join public.courses c on c.id=m.course_id left join manifest e on e.sort=m.sort_order where c.slug='music-business' and m.sort_order between 1 and 48 and (e.sort is null or m.title is distinct from e.title)
), child_conflicts as (
 select count(*)::int n from manifest e join public.courses c on c.slug='music-business' join public.course_levels l on l.course_id=c.id and l.level_number=e.level join public.course_modules m on m.course_level_id=l.id and m.level_module_number=e.num
 ...
 union all select 'IDENTITY','EXISTING_MUSIC_BUSINESS_SHELL_EMPTY',case when modules=0 and lessons=0 and activities=0 and enrollments=0 and submissions=0 and lesson_progress=0 and xp_ledger=0 and certificates=0 then 'PASS' else 'BLOCK' end,format('modules=%s/0; lessons=%s/0; activities=%s/0; enrollments=%s/0; submissions=%s/0; lesson_progress=%s/0; xp_ledger=%s/0; certificates=%s/0',modules,lessons,activities,enrollments,submissions,lesson_progress,xp_ledger,certificates) from shell_empty
