-- JPAC Creative Studio lab-tools seed post-validation. READ ONLY.
begin transaction read only;

with expected_tools(slug,name,singing_link) as (
  values
    ('vocal-practice-planner','Vocal Practice Planner',true),
    ('performance-prep-checklist','Performance Prep Checklist',true),
    ('assignment-practice-builder','Assignment Practice Builder',true),
    ('script-scene-rehearsal-tool','Script & Scene Rehearsal Tool',false),
    ('dance-rehearsal-tracker','Dance Rehearsal Tracker',false),
    ('songwriting-idea-pad','Songwriting Idea Pad',false),
    ('video-shot-planner','Video Shot Planner',false),
    ('portfolio-builder-checklist','Portfolio Builder Checklist',true)
), tool_state as (
  select
    (select count(*) from public.lab_tools t join expected_tools e on e.slug=t.slug and e.name=t.name)::int expected_tools,
    (select count(*) from public.lab_tools t join expected_tools e on e.slug=t.slug
      where t.status='testing' and t.tool_type='built_in' and t.launch_url is null and t.xp_reward=0
        and t.admin_notes='Seeded by 202608240001_jpac_lab_tools_seed')::int safe_tools,
  ...
  select e.slug,e.modules expected,count(distinct c.id)::int course_rows,count(m.id)::int actual from expected_courses e
  left join public.courses c on c.slug=e.slug and c.title=e.title left join public.course_modules m on m.course_id=c.id
  group by e.slug,e.modules
), protected_summary as (
  select count(*) filter(where course_rows<>1 or actual<>expected)::int blockers,jsonb_object_agg(slug,jsonb_build_object('actual',actual,'expected',expected) order by slug) counts from protected_catalog
), assignment_swap as (
  ...
  select count(*)::int tables from information_schema.tables where table_schema='public' and table_name in('community_posts','community_comments','community_reactions','community_moderation_actions','community_reports')
), findings(report_section,code,result,details) as (
  select 'CATALOG','EXPECTED_TOOLS',case when expected_tools=8 then 'PASS' else 'BLOCK' end,format('expected names/slugs=%s/8',expected_tools) from tool_state
  union all select 'CATALOG','SAFE_TESTING_VISIBILITY',case when safe_tools=8 and ready_tools=0 then 'PASS' else 'BLOCK' end,format('exact testing/built-in/null-launch/zero-XP seed rows=%s/8; globally ready tools=%s/0',safe_tools,ready_tools) from tool_state
  union all select 'COURSE ASSIGNMENTS','SINGING_ONLY',case when singing_links=4 and other_links=0 then 'PASS' else 'BLOCK' end,format('Singing seed links=%s/4; other-course seed links=%s/0',singing_links,other_links) from tool_state
...
union all select 'READINESS','OVERALL',case when blockers=0 then 'PASS' else 'BLOCK' end,case when blockers=0 then 'PASS: SEED INTACT; TOOLS REMAIN HIDDEN PENDING REVIEW' else 'BLOCK: REVIEW OR ROLLBACK SEED' end from summary
