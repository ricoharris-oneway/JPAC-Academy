...
union all select 'SAFETY','PROTECTED_TABLES',case when count(*)=7 then 'PASS' else 'BLOCK' end,format('protected tables present=%s/7',count(*)) from information_schema.tables where table_schema='public' and table_name in ('courses','modules','enrollments','submissions','xp_ledger','lesson_progress','certificates')),
