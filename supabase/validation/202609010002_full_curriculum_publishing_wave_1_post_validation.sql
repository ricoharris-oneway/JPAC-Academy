...
begin
  ...
  select md5(coalesce(string_agg(id::text||':'||status||':'||coalesce(approved_at::text,''),E'\n' order by id),'')) into actual_hash from public.course_levels; if actual_hash<>'057fcce9d1d8224d80bdc92c54b77b6a' then raise exception 'Course-level status/approval changed'; end if;
