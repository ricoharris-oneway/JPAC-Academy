-- JPAC Music Production/Songwriting 48-module rollout preflight. READ ONLY.
...
with manifest as (select * from jsonb_to_recordset('[
 ...
 select count(*)::int n,(array_agg(id order by id::text))[1] id from public.courses where slug='music-production-songwriting'
), sdi as (
 ...
 select count(*)::int n from public.course_levels l join public.courses c on c.id=l.course_id where c.slug='music-production-songwriting' and (l.level_number not between 1 and 4 or l.title is distinct from (array['Beginner','Intermediate','Advanced','Master'])[l.level_number] or l.description is distinct from 'Draft Music Production/Songwriting level; Program PDF source review only.' or l.learning_objectives is distinct from array['Develop source-aligned songwriting, arrangement, production, recording, mixing, collaboration, ownership, and professional delivery skills safely.'] or l.status<>'draft' or l.core_xp_target<>7500 or l.review_notes not like '%inactive certificate intent: '||(array['JPAC Music Creator Certificate','JPAC Song Builder Certificate','JPAC Producer & Songwriter Certificate','JPAC Master Creative Producer Certificate'])[l.level_number]||'.%' or l.approved_by is not null or l.approved_at is not null)
), unexpected_modules as (
 select count(*)::int n from public.course_modules m join public.courses c on c.id=m.course_id join public.course_levels l on l.id=m.course_level_id left join manifest e on e.level=l.level_number and e.num=m.level_module_number and e.sort=m.sort_order and e.title=m.title where c.slug='music-production-songwriting' and e.num is null
), module_conflicts as (
 select count(*)::int n from manifest e join public.courses c on c.slug='music-production-songwriting' left join public.course_levels l on l.course_id=c.id and l.level_number=e.level join public.course_modules m on m.course_id=c.id and (m.sort_order=e.sort or (m.course_level_id=l.id and m.level_module_number=e.num))
 where m.course_level_id is distinct from l.id or m.level_module_number is distinct from e.num or m.sort_order is distinct from e.sort or m.title is distinct from e.title or m.description is distinct from 'Develop and demonstrate source-aligned Music Production/Songwriting skills for '||e.title||'.' or m.short_intro is distinct from 'Draft source-aligned Music Production/Songwriting module.' or m.status<>'draft' or m.xp_value<>625 or m.core_xp<>625 or m.intro_core_xp<>50 or m.video_core_xp<>100 or m.assignment_core_xp<>350 or m.mastery_core_xp<>125 or m.core_unlock_threshold<>438 or m.bonus_xp_available<>0 or m.jpac_tool_activity is distinct from '{}'::jsonb or m.real_world_activity is distinct from jsonb_build_object('title',e.title||' Guided Practice','instructions','Practice the approved creative objective using teacher-approved DAW-neutral, instrument-neutral, written, performed, or supplied-material alternatives. Named products and external accounts are optional examples only.') or m.career_connection is distinct from '' or m.portfolio_moment or m.video_brief is distinct from 'MEDIA NEEDS REVIEW: no media is activated.' or m.review_notes not like '%MEDIA NEEDS REVIEW%' or m.review_notes not like '%NEEDS CATALOG REVIEW%' or m.review_notes not like '%RIGHTS/LEGAL REVIEW%' or m.review_notes not like '%COLLABORATION/CONSENT REVIEW%' or m.review_notes not like '%THIRD-PARTY TOOL REVIEW%' or m.review_notes not like '%COPYRIGHT/OWNERSHIP REVIEW%' or m.review_notes not like '%SCOPE REVIEW%' or m.review_notes not like '%JPAC-native submission%' or (e.num=12 and m.review_notes not like '%Approved level capstone%') or m.lab_tool_id is not null or m.active_instructional_media_id is not null or m.primary_video_url is not null or m.approved_by is not null or m.approved_at is not null
), sort_conflicts as (
 select count(*)::int n from public.course_modules m join public.courses c on c.id=m.course_id left join manifest e on e.sort=m.sort_order where c.slug='music-production-songwriting' and m.sort_order between 1 and 48 and (e.sort is null or m.title is distinct from e.title)
), child_conflicts as (
 select count(*)::int n from manifest e join public.courses c on c.slug='music-production-songwriting' join public.course_levels l on l.course_id=c.id and l.level_number=e.level join public.course_modules m on m.course_level_id=l.id and m.level_module_number=e.num
 where (exists(select 1 from public.lessons where module_id=m.id) or exists(select 1 from public.activities where module_id=m.id)) and ((select count(*) from public.lessons where module_id=m.id)<>3 or (select count(*) from public.lessons where module_id=m.id and status='draft' and xp_value=0 and title in(e.title||': Creative Foundations and Safety',e.title||': Guided Production Application',e.title||': Professional Delivery and Reflection'))<>3 or (select count(*) from public.activities where module_id=m.id)<>2 or (select count(*) from public.activities where module_id=m.id and title=e.title||' Guided Practice' and status='draft' and activity_type='practice' and not required and xp_reward=0 and xp_type='bonus')<>1 or (select count(*) from public.activities where module_id=m.id and title=e.title||' Core Challenge' and status='draft' and activity_type='performance' and required and xp_reward=350 and xp_type='core' and passing_score=70 and allows_resubmission and jsonb_array_length(rubric->'criteria')=5 and (select sum((x->>'weight')::int) from jsonb_array_elements(rubric->'criteria') x)=100)<>1)
), assets as (
 ...
 select 'IDENTITY' section,'MUSIC_PRODUCTION_SONGWRITING_IDENTITY' code,case when n=1 then 'PASS' else 'BLOCK' end result,format('canonical Music Production/Songwriting course rows=%s; id=%s',n,coalesce(id::text,'NULL')) details from course_identity
 ...
 union all select 'EVIDENCE','MUSIC_PRODUCTION_SONGWRITING_DEPENDENCIES',case when n=0 then 'PASS' else 'BLOCK' end,format('Music Production/Songwriting evidence/student dependencies=%s',n) from deps
...
union all select 'READINESS','OVERALL',case when n=0 then 'PASS' else 'BLOCK' end,case when n=0 then 'PASS: READY FOR MUSIC PRODUCTION/SONGWRITING DRAFT ROLLOUT MIGRATION' else 'BLOCK: DO NOT RUN MIGRATION' end from blockers
