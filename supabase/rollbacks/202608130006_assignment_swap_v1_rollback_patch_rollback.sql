...
begin
  if exists(select 1 from public.curriculum_assignment_swap_operations where rollback_of='149f6b67-a615-4d17-b2ac-75879e0467dc'::uuid) then
    raise exception 'Refusing to restore known-defective rollback definition after a rollback audit row exists';
  end if;
...
-- WARNING: this restores the prior non-canonical passing_score hash behavior.
...
begin
  ...
  v_current:=jsonb_build_object('title',v_activity.title,'description',v_activity.description,'instructions',v_activity.instructions,'submission_type',v_activity.submission_type,'passing_score',v_activity.passing_score,'allows_resubmission',v_activity.allows_resubmission,'rubric',v_activity.rubric); v_hash:=pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(v_current::text,'UTF8')),'hex');
...
comment on function public.curriculum_rollback_assignment_swap_v1(uuid) is 'Assignment Swap v1 rollback restored to pre-202608130006 behavior; known passing_score canonical hash defect is present.';
