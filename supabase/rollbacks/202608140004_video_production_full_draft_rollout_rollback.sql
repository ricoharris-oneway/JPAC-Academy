-- JPAC Video Production 49-module draft rollout rollback. Removes exact batch-created rows only.
begin;
do $$
declare v_course uuid; v_marker constant text:='Video Production full draft rollout 202608140004'; v_count int;
begin
 ...
 delete from public.course_levels l where l.course_id=v_course and l.review_notes=v_marker||'; batch-created level; metadata target reconciliation deferred.' and not exists(select 1 from public.course_modules m where m.course_level_id=l.id);
 raise notice 'Removed % exact batch-created Video Production modules; compatible pre-existing rows preserved',v_count;
end $$;
commit;
