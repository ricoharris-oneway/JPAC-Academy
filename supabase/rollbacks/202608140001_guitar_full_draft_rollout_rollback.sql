-- JPAC Guitar 50-module draft rollout rollback.
begin;
do $$
declare v_course uuid; v_marker constant text:='Guitar full draft rollout 202608140001'; v_count int;
begin
 ...
 or exists(select 1 from public.course_progress p where p.course_id=v_course)
 ...
 raise notice 'Removed % exact batch-created Guitar modules; compatible pre-existing rows preserved',v_count;
end $$;
commit;
