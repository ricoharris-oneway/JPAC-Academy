-- JPAC Dance 47-module draft rollout rollback. Removes batch-created Dance rows only.
begin;
do $$
declare v_course uuid; v_marker constant text:='Dance full draft rollout 202608140003'; v_count int;
begin
 if (select count(*) from public.courses where slug='dance')<>1 then raise exception 'Expected exactly one canonical dance course'; end if;
 select id into v_course from public.courses where slug='dance' for share;
 ...
 raise notice 'Removed % exact batch-created Dance modules; compatible pre-existing rows preserved',v_count;
end $$;
commit;
