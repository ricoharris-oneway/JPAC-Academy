...
create table public.curriculum_assignment_swap_operations(
  id uuid primary key default gen_random_uuid(),
  target_course_id uuid not null references public.courses(id) on delete restrict,
  target_module_id uuid not null references public.course_modules(id) on delete restrict,
  target_activity_id uuid not null references public.activities(id) on delete restrict,
  assignment_role text not null check(assignment_role in('practice','core_challenge')),
  operation_kind text not null default 'swap' check(operation_kind in('swap','rollback')),
  before_payload jsonb not null check(jsonb_typeof(before_payload)='object'),
  ...
  rollback_of uuid references public.curriculum_assignment_swap_operations(id) on delete restrict,
  status text not null default 'applied' check(status='applied'),
  created_at timestamptz not null default now(),
  check((operation_kind='swap' and rollback_of is null) or (operation_kind='rollback' and rollback_of is not null)),
  unique(rollback_of)
);

create index curriculum_assignment_swap_target_idx on public.curriculum_assignment_swap_operations(target_module_id,target_activity_id,created_at desc);
alter table public.curriculum_assignment_swap_operations enable row level security;
revoke all on public.curriculum_assignment_swap_operations from public,anon,authenticated;
grant select on public.curriculum_assignment_swap_operations to authenticated;
create policy curriculum_assignment_swap_admin_read on public.curriculum_assignment_swap_operations for select to authenticated using(public.is_admin());

create function public.jpac_block_assignment_swap_audit_mutation()
returns trigger language plpgsql security definer set search_path=public as $$
begin raise exception 'Assignment Swap audit records are append-only'; end $$;
create trigger curriculum_assignment_swap_audit_immutable before update or delete on public.curriculum_assignment_swap_operations for each row execute function public.jpac_block_assignment_swap_audit_mutation();

create function public.curriculum_swap_module_assignment_v1(swap_payload jsonb)
...
create function public.curriculum_rollback_assignment_swap_v1(operation_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
...
revoke all on function public.jpac_block_assignment_swap_audit_mutation() from public,anon,authenticated;
revoke all on function public.curriculum_swap_module_assignment_v1(jsonb) from public,anon;
revoke all on function public.curriculum_rollback_assignment_swap_v1(uuid) from public,anon;
...
comment on table public.curriculum_assignment_swap_operations is 'Append-only administrative audit for draft-only Assignment Swap v1 operations.';
commit;
