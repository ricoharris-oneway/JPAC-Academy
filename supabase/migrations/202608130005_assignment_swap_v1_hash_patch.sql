...
begin
  ...
  if exists(select 1 from public.curriculum_assignment_swap_operations) then
    raise exception 'Assignment Swap audit history exists; do not replace hash implementation without review';
  end if;
  if to_regprocedure('pg_catalog.sha256(bytea)') is null then
    raise exception 'pg_catalog.sha256(bytea) is unavailable';
  end if;
end $$;
...
begin
  ...
  v_before_hash:=pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(v_before::text,'UTF8')),'hex');
  ...
  v_after_hash:=pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to(v_after::text,'UTF8')),'hex');
