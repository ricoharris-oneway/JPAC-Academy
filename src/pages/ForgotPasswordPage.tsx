...
export function ForgotPasswordPage(){
  ...
  async function submit(event:FormEvent){event.preventDefault();if(!supabase){setMessage('Supabase is not configured.');return}setBusy(true);const{error}=await supabase.auth.resetPasswordForEmail(email.trim().toLowerCase(),{redirectTo:authCallbackUrl('/set-password')});setBusy(false);setMessage(error?.message||'If an Academy account exists for that email, a password reset link is on its way.')}
