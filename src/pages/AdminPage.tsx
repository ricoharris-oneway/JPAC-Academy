...
export function AdminPage(){
  ...
  async function issueCertificate(){
    if(!supabase||!selectedUser||!selectedCourse)return;
    const course=courses.find(x=>x.id===selectedCourse);
    const instructor=teachers[0]?.display_name||profile?.display_name||'JPAC Academy Instructor';
    const{error}=await supabase.rpc('admin_issue_completion_certificate',{target_student:selectedUser,target_course:selectedCourse,completion_on:new Date().toISOString().slice(0,10),grade_value:'A',score_value:95,hours_value:40,level_value:'Level 1',instructor_value:instructor});
    setMessage(error?.message||`Certificate queued for ${course?.title||'course'}.`);
  }
