...
export function PaymentLedgerPage() {
  ...
  const familyPrefill=searchParams.get('fromFamilyBilling')==='1';
  const expectedAmount=searchParams.get('expectedAmount')||'';
  const billingMonth=searchParams.get('billingMonth')||'';
  const pricingSource=searchParams.get('pricingSource')||'';
  const courseTitle=searchParams.get('courseTitle')||'';
  const overrideReason=searchParams.get('overrideReason')||'';
  ...
  async function loadLedger(found: StudentLookup) {
    if (!supabase) return [] as EnrollmentChoice[];
    ...
    const enrollmentList=(enrollmentRows || []) as EnrollmentChoice[];
    setEnrollments(enrollmentList); setEntries(ledger); return enrollmentList;
  }

  function applyFamilyPrefill(enrollmentList:EnrollmentChoice[]) {
    if(!familyPrefill)return;
    const matchedCourse=courses.find(course=>course.title.trim().toLowerCase()===courseTitle.trim().toLowerCase());
    const matchedEnrollment=matchedCourse?enrollmentList.find(item=>item.course_id===matchedCourse.id):undefined;
    const monthLabel=billingMonth?new Date(`${billingMonth}-01T12:00:00`).toLocaleDateString(undefined,{month:'long',year:'numeric'}):'selected month';
    const note=[`Family Billing verification for ${monthLabel}.`,`Expected Wix tuition: $${Number(expectedAmount||0).toFixed(2)}.`,pricingSource?`Pricing source: ${pricingSource}.`:'',overrideReason?`Override reason: ${overrideReason}.`:''].filter(Boolean).join(' ');
    setForm(current=>({...current,courseId:matchedCourse?.id||'',enrollmentId:matchedEnrollment?.id||'',paymentStatus:'verified',paymentMethod:'wix_online',purchaseType:'first_course_purchase',amount:expectedAmount,currency:'USD',paymentDate:today(),externalSource:'Wix',internalNote:note}));
    setMessage('Family Billing details loaded. Confirm the Wix payment reference and payment date, then save the verified payment.');
  }
  ...
  async function lookup(targetEmail?:string) {
    ...
    try { const { data, error: lookupError } = await supabase.from('profiles').select('id,display_name,email,role').ilike('email', value).eq('role', 'student').maybeSingle(); if (lookupError) throw lookupError; if (!data) return setError('student_missing: No existing student profile was found for that email.'); const found = data as StudentLookup; setStudent(found);localStorage.setItem(ACTIVE_STUDENT_KEY,value);const enrollmentList=await loadLedger(found);if(familyPrefill)applyFamilyPrefill(enrollmentList);else setSearchParams({email:value},{replace:true}); }
  ...
  async function save() {
    ...
    if(familyPrefill && !form.externalReference.trim() && !form.referenceNumber.trim()) return setError('Enter the Wix transaction, order, or subscription reference before verifying this payment.');
    setBusy(true); try { await savePaymentLedgerEntry(student.id, form); await loadLedger(student); setForm(blankForm()); setMessage('Wix payment verified and saved to the JPAC Payment Ledger.'); if(familyPrefill)setSearchParams({email:student.email||email},{replace:true}); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to save the payment record.'); } finally { setBusy(false); }
  }

  return <div className="payment-ledger-page"><header className="page-hero"><div><div className="eyebrow">Staff operations</div><h1 className="page-title">Student Payment Ledger</h1><p className="muted">Wix collects payment. JPAC verifies the payment, preserves the Wix reference, and clears the student's operational payment requirement.</p></div></header>
    {(error || message) && <div className={error ? 'admin-message error' : 'admin-message'} role="status">{error || message}</div>}
    {familyPrefill&&student&&<section className="card card-pad" style={{marginBottom:18,border:'1px solid rgba(245,196,81,.45)'}}><div className="eyebrow">Family Billing verification</div><h2>Confirm the Wix payment</h2><p><strong>Expected amount:</strong> ${Number(expectedAmount||0).toFixed(2)}{billingMonth?` · ${new Date(`${billingMonth}-01T12:00:00`).toLocaleDateString(undefined,{month:'long',year:'numeric'})}`:''}</p><p><strong>Program:</strong> {courseTitle||'Not specified'} · <strong>Pricing:</strong> {pricingSource||'Standard'}</p><p className="muted">Compare this expected amount with Wix. Enter the Wix transaction/order/subscription reference below and save. JPAC does not charge the card.</p></section>}
    ...
    {student && <div className="payment-ledger-grid"><section className="card card-pad ledger-form"><div className="ledger-entry-heading"><h2>{form.id ? 'Edit payment record' : familyPrefill?'Verify Wix payment':'Add payment record'}</h2>{form.id && <button className="button button-secondary" type="button" onClick={() => setForm(blankForm())}>Cancel edit</button>}</div>
