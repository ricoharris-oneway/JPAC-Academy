...
export function MemberHomePage() {
  ...
  return <div className="member-launch">
    ...
    <MemberRow title="Featured Programs">{memberPrograms.map((program, i) => <Link className="member-program" to={`/programs#${program.slug}`} key={program.slug}><img src={`/assets/${program.image}`} alt="" loading="lazy" /><div><span>{program.category}</span><h3>{program.title}</h3><p>Explore program <b>↗</b></p></div><span className="program-number">{String(i + 1).padStart(2, '0')}</span></Link>)}</MemberRow>
    <MemberRow title="Career Paths">{careerPaths.map(path => <Link className="member-career" to="/career-pathing" key={path.id}><img src="/assets/academy-hero.svg" alt="" loading="lazy" /><span className="career-symbol" aria-hidden="true">{path.icon}</span><small>YOUR FUTURE IN FOCUS</small><h3>{path.title}</h3><p>{path.outcome}</p><b>Explore your path ↗</b></Link>)}</MemberRow>
    <MemberRow title="JPAC Tools">{memberTools.map(tool => <Link className="member-tool" to={tool.to} key={tool.title}><div className="tool-preview" aria-hidden="true"><img src="/assets/academy-hero.svg" alt="" loading="lazy" /><span>{tool.icon}</span></div><h3>{tool.title}</h3><p>{tool.description}</p><b>Explore →</b></Link>)}</MemberRow>
    <MemberRow title="Unlock Premium Courses">{memberPrograms.map(program => { const enrolled = access.find(course => course.slug === program.slug || course.title.toLowerCase() === program.title.toLowerCase()); return <Link className="member-locked" to={enrolled ? `/courses/${enrolled.course_id}` : `/programs#${program.slug}`} key={program.slug}><img src={`/assets/${program.image}`} alt="" loading="lazy" /><div><span className="member-pill">{enrolled ? '✓ Enrolled · Open course' : '🔒 Locked until enrolled'}</span><h3>{program.title}</h3><p>{enrolled ? 'Continue your Academy learning.' : 'Staff verification required'}</p><small>{enrolled ? 'Your approved program' : 'Payment or approved access required'}</small></div></Link>; })}</MemberRow>
    <EnrollmentOffer />
...
export function ExploreProgramsPage() {
  return <div className="member-launch"><header className="member-page-heading"><span className="member-kicker">FIND YOUR NEXT CHAPTER</span><h1>Explore Programs</h1><p>Start with your curiosity. Enrollment is approved for each course by JPAC staff.</p></header><div className="member-program-grid">{memberPrograms.map(program => <article className="member-program-detail" id={program.slug} key={program.slug}><img src={`/assets/${program.image}`} alt="" loading="lazy" /><div><span className="member-kicker">{program.category}</span><h2>{program.title}</h2><p>{program.description}</p><p className="member-note">{courseLockedMessage}</p><a className="button button-secondary" href={`${enrollmentRequestUrl}&body=${encodeURIComponent(`I would like to learn about enrollment in ${program.title}.`)}`}>Ask about this program ↗</a></div></article>)}</div><EnrollmentOffer /></div>;
}
