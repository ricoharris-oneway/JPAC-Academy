...
const careerImages: Record<string, string> = {
  performance: '/creative-assets/jpac-showcase-stage.webp',
  'music-creation': '/creative-assets/jpac-creative-spaces-grid.webp',
  'stage-screen': '/creative-assets/jpac-video-editing-studio.webp',
  'creative-business': '/creative-assets/jpac-main-lobby.webp',
  'education-leadership': '/creative-assets/jpac-dance-studio.webp',
};

const toolImages: Record<string, string> = {
  'JPAC Coach': '/creative-assets/jpac-main-lobby.webp',
  'Creative Studio': '/creative-assets/jpac-creative-spaces-grid.webp',
  'Practice Submissions': '/creative-assets/jpac-singing-studio.webp',
  'Career Pathing': '/creative-assets/jpac-showcase-stage.webp',
  'Video Finder Helper': '/creative-assets/jpac-video-editing-studio.webp',
  'Certificates & Portfolio': '/creative-assets/jpac-main-lobby.webp',
  Community: '/creative-assets/jpac-dance-studio.webp',
};
...
export function MemberHomePage() {
  ...
  return <div className="member-launch">
    ...
    <MemberRow title="Career Paths">{careerPaths.map(path => <Link className="member-career" to="/career-pathing" key={path.id}><img src={careerImages[path.category]} alt="" loading="lazy" /><span className="career-symbol" aria-hidden="true">{path.icon}</span><small>YOUR FUTURE IN FOCUS</small><h3>{path.title}</h3><p>{path.outcome}</p><b>Explore your path ↗</b></Link>)}</MemberRow>
    <MemberRow title="JPAC Tools">{memberTools.map(tool => <Link className="member-tool" to={tool.to} key={tool.title}><div className="tool-preview" aria-hidden="true"><img src={toolImages[tool.title]} alt="" loading="lazy" /><span>{tool.icon}</span></div><h3>{tool.title}</h3><p>{tool.description}</p><b>Explore →</b></Link>)}</MemberRow>
