...
export function MemberHomePage() {
  ...
  const path = careerPathBySlug(selectedPath || '');
  ...
  return <div className="member-launch">
    ...
    <div className="member-welcome"><span className="member-pill">FREE MEMBERSHIP · WELCOME IN</span><p>Your direction: <strong>{path?.name || 'Your creative future'}</strong></p><Link to="/courses">My Learning →</Link></div>
    ...
    <MemberRow title="Career Paths">{careerPaths.map(path => <Link className="member-career" to={`/career-paths/${path.slug}`} key={path.slug}><span aria-hidden="true">{path.icon}</span><small>YOUR FUTURE IN FOCUS</small><h3>{path.name}</h3><p>{path.outcome}</p><b>Explore your path ↗</b></Link>)}</MemberRow>
