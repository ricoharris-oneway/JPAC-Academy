...
export function CertificateCenterPage(){
  const{profile}=useAuth();const[certificates,setCertificates]=useState<Certificate[]>([]);const[portfolio,setPortfolio]=useState<Portfolio[]>([]);const[message,setMessage]=useState('');const[loading,setLoading]=useState(true);
  ...
  async function load(){
    ...
    const[{data:c,error:ce},{data:p,error:pe}]=await Promise.all([certificateQuery,portfolioQuery]);setCertificates((c as Certificate[])||[]);setPortfolio((p as Portfolio[])||[]);setMessage(ce?.message||pe?.message||'');setLoading(false)
  }
