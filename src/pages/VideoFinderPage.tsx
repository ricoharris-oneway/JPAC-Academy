...
export function VideoFinderPage() {
  ...
  const saveVideo = async (row: VideoFinderRow) => {
    ...
    setModules((values) => values.map((module) => module.id === row.moduleId ? { ...module, primary_video_url: normalized?.normalizedUrl || module.primary_video_url, video_title: draft.title.trim(), video_duration_seconds: Number(draft.duration) } : module));
