  ...
  return <div className={`career-pathing-page career-experience-v2${paused ? ' career-motion-paused' : ''}`}>
    ...
    <section className="career-path-roadmap" id="career-selected-path" tabIndex={-1} aria-labelledby="career-roadmap-title">
      ...
      <div className="career-roadmap-grid" aria-label={`${selectedPath.title} roadmap`}>
        ...
        <ol className="career-journey-rail">{journey.map(([title, description], index) => { const status = index === 0 ? 'completed' : index === 1 ? 'active' : 'locked'; return <li className={`roadmap-node ${status}`} data-state={status} key={title}><span aria-hidden="true">{status === 'completed' ? '✓' : status === 'locked' ? '○' : String(index + 1).padStart(2, '0')}</span><div><strong>{title}</strong><small>{description}</small><em>{status}</em></div></li>; })}</ol>
      </div>
    </section>
