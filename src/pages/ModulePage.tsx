...
export function ModulePage(){
  ...
  async function trackVideo(event:React.SyntheticEvent<HTMLVideoElement>){if(!supabase)return;const video=event.currentTarget;if(!Number.isFinite(video.duration)||video.duration<=0)return;const watched=Math.floor(video.currentTime);const ratio=video.currentTime/video.duration;if(watched-lastVideoSecond.current<5&&ratio<.9)return;lastVideoSecond.current=watched;const{error}=await supabase.rpc('jpac_record_module_video_progress',{target_module:moduleId,watched,duration:Math.floor(video.duration)});if(error)setMessage(error.message);else if(ratio>=.9&&Number(completion?.video_percent||0)<90)await load()}
