...
export function ModulePage(){
  ...
  return <div className="creator-mission"><Link className="text-link" to={`/courses/${course.id}`}>← {course.title}</Link>
    ...
    {requiredActivity&&<section id="mission-create" tabIndex={-1} className="mission-card creative-challenge"><div className="creative-challenge-head"><div><div className="mission-section-label">Create it</div><h2>{requiredActivity.title}</h2></div><strong>{requiredActivity.xp_reward} Core XP</strong></div><p>{requiredActivity.description}</p><p className="muted">{requiredActivity.instructions}</p><div className="submission-state"><strong>{requiredAttempts.length===0?'Assignment available':requiredAttempts[0]?.status==='approved'?'Approved / completed':requiredAttempts[0]?.status==='revision_requested'?'Needs revision':'Submitted / awaiting review'}</strong><small>{requiredAttempts.length===0?'Submit a take when you are ready. Viewing this card never awards credit.':requiredAttempts[0]?.status==='approved'?'Required review passed; assignment credit is recorded.':'Credit is awarded only after the required staff review.'}</small></div><SubmissionHistory attempts={requiredAttempts}/><label className="practice-drop"><input type="file" accept="audio/*,video/*" onChange={event=>setFile(event.target.files?.[0]||null)}/><strong>{file?.name||'Choose an audio or video take'}</strong><small>{file?`${Math.max(.01,file.size/1024/1024).toFixed(2)} MB · ready to upload`:'Your file remains private and previous attempts are never overwritten.'}</small></label><button className="button button-primary" disabled={!file||busy} onClick={()=>void submit(requiredActivity)}>{busy?'Uploading…':requiredAttempts.length?`Submit attempt ${Math.max(...requiredAttempts.map(item=>item.attempt_number))+1}`:'Submit creative challenge'}</button></section>}
