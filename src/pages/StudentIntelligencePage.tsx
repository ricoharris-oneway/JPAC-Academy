...
type Student={id:string;display_name:string|null;email:string|null;total_xp:number|null};
...
const studentName=(student:Student)=>student.display_name?.trim()||student.email?.split('@')[0]||'JPAC Student';
const studentInitials=(student:Student)=>studentName(student).split(/\s+/).map(part=>part[0]).join('').slice(0,2).toUpperCase();
const facebookPageUrl=import.meta.env.VITE_JMONE_FACEBOOK_PAGE_URL?.trim()||'';
function validFacebookPageUrl(value:string){try{const url=new URL(value);return url.protocol==='https:'&&(url.hostname==='facebook.com'||url.hostname.endsWith('.facebook.com'))}catch{return false}}

function FacebookTimeline({isStaff}:{isStaff:boolean}){
  const configured=validFacebookPageUrl(facebookPageUrl);
  // Phase 1 embed only. Graph API sync may be added later after Meta app review and token setup.
  const embedUrl=configured?`https://www.facebook.com/plugins/page.php?href=${encodeURIComponent(facebookPageUrl)}&tabs=timeline&width=500&height=620&small_header=false&adapt_container_width=true&hide_cover=false&show_facepile=false`:'';
  if(!configured&&!isStaff)return null;
  return <section className="card journey-section" aria-labelledby="jmone-live-updates-title" style={{overflow:'hidden',background:'radial-gradient(circle at 92% 8%,rgba(255,215,120,.12),transparent 28%),linear-gradient(145deg,rgba(42,15,65,.94),rgba(8,4,14,.98))'}}><div className="eyebrow">From the community</div><h2 id="jmone-live-updates-title">J. Moné Live Updates</h2><p className="muted">Latest public updates from J. Moné’s official Facebook page.</p>{configured?<div style={{width:'100%',maxWidth:500,minHeight:620,margin:'20px auto 0',overflow:'hidden',borderRadius:18,border:'1px solid rgba(255,215,120,.2)',background:'#12071b'}}><iframe title="J. Moné official Facebook Page timeline" src={embedUrl} width="500" height="620" style={{display:'block',width:'100%',maxWidth:500,height:620,border:0}} scrolling="no" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen loading="lazy" referrerPolicy="strict-origin-when-cross-origin"/></div>:<div className="journey-empty" role="status">Set <code>VITE_JMONE_FACEBOOK_PAGE_URL</code> to the HTTPS URL of J. Moné’s official Facebook page to enable public updates. No Facebook token or login is required.</div>}</section>;
}

export function StudentIntelligencePage(){
  const{user,profile}=useAuth();
  const[students,setStudents]=useState<Student[]>([]);const[twins,setTwins]=useState<Twin[]>([]);const[enrollmentProgress,setEnrollmentProgress]=useState<EnrollmentProgress[]>([]);const[preferences,setPreferences]=useState<Preference[]>([]);const[strengths,setStrengths]=useState<Strength[]>([]);const[recommendations,setRecommendations]=useState<Recommendation[]>([]);const[timeline,setTimeline]=useState<JourneyEvent[]>([]);const[selected,setSelected]=useState('');const[message,setMessage]=useState('');const[error,setError]=useState('');const[loading,setLoading]=useState(true);
  const isStaff=Boolean(profile&&['teacher','admin','developer'].includes(profile.role));
  async function load(){
    if(!user){setLoading(false);return}
    if(!supabase){setError('Student Intelligence is temporarily unavailable because the Academy data service is not configured.');setLoading(false);return}
    setLoading(true);
    setError('');
    try{
      ...
      const twinQuery=isStaff?supabase.from('student_digital_twins').select('*'):supabase.from('student_digital_twins').select('*').eq('student_id',user.id);
      const enrollmentQuery=isStaff?supabase.from('enrollments').select('student_id,progress,status').eq('status','active'):supabase.from('enrollments').select('student_id,progress,status').eq('status','active').eq('student_id',user.id);
      const preferenceQuery=isStaff?supabase.from('student_learning_preferences').select('student_id,visual_score,auditory_score,kinesthetic_score,reading_score,preferred_pace,preferred_feedback'):supabase.from('student_learning_preferences').select('student_id,visual_score,auditory_score,kinesthetic_score,reading_score,preferred_pace,preferred_feedback').eq('student_id',user.id);
      const strengthQuery=isStaff?supabase.from('student_strengths').select('id,student_id,skill_name,confidence').order('confidence',{ascending:false}):supabase.from('student_strengths').select('id,student_id,skill_name,confidence').eq('student_id',user.id).order('confidence',{ascending:false});
      const recommendationQuery=isStaff?supabase.from('aria_recommendations').select('id,student_id,title,rationale,priority,status,recommendation_type').eq('status','active').order('priority',{ascending:false}):supabase.from('aria_recommendations').select('id,student_id,title,rationale,priority,status,recommendation_type').eq('status','active').eq('student_id',user.id).order('priority',{ascending:false});
      const timelineQuery=isStaff?supabase.from('student_timeline').select('id,student_id,event_type,title,description,occurred_at').order('occurred_at',{ascending:false}).limit(60):supabase.from('student_timeline').select('id,student_id,event_type,title,description,occurred_at').eq('student_id',user.id).order('occurred_at',{ascending:false}).limit(60);
      const results=await Promise.all([studentFilter,twinQuery,enrollmentQuery,preferenceQuery,strengthQuery,recommendationQuery,timelineQuery]);
      const failed=results.find(result=>result.error)?.error;
      if(failed)throw failed;
      const[{data:s},{data:t},{data:ep},{data:p},{data:st},{data:r},{data:tl}]=results;
      const studentRows=(s as Student[])||[];setStudents(studentRows);setTwins((t as Twin[])||[]);setEnrollmentProgress((ep as EnrollmentProgress[])||[]);setPreferences((p as Preference[])||[]);setStrengths((st as Strength[])||[]);setRecommendations((r as Recommendation[])||[]);setTimeline((tl as JourneyEvent[])||[]);setSelected(current=>studentRows.some(item=>item.id===current)?current:studentRows[0]?.id||'');
    }catch(loadError){
      setError(loadError instanceof Error?loadError.message:'Student Intelligence could not load. Please try again.');
  ...
  async function refreshTwin(){if(!supabase||!selected)return;setMessage('Refreshing production learning evidence…');const{error:refreshError}=await supabase.rpc('refresh_student_digital_twin',{target_student:selected,refresh_reason:'Staff evidence refresh'});setMessage(refreshError?.message||'Learning evidence refreshed.');if(!refreshError)await load()}
  if(loading)return <section className="card card-pad" role="status" aria-live="polite"><h2>Loading Student Intelligence…</h2><p className="muted">Gathering verified Academy progress and learning evidence.</p><SkeletonCards count={5}/></section>;
  if(error)return <section className="card card-pad" role="alert"><div className="eyebrow">Student Intelligence</div><h2>We couldn't load this page.</h2><p className="muted">{error}</p><button className="button button-primary" onClick={()=>void load()}>Try again</button></section>;
  if(!student)return <div className="student-intelligence-empty"><EmptyState icon="🌟" title="No creative journey available yet" detail={isStaff?'Create or enroll a student before learning evidence can be analyzed.':'Your learning evidence will appear here after you begin an enrolled course.'} actionLabel={isStaff?'Open Admissions':undefined} actionTo={isStaff?'/manual-student':undefined}/></div>;
  const displayName=studentName(student);const firstName=displayName.split(/\s+/)[0];
  return <div className="creative-journey">
    <WorkspaceHero eyebrow={isStaff?'Student Intelligence · Creative Journey':'My Growth · Creative Journey'} title={isStaff?`${displayName}'s Creative Journey`:`Welcome back, ${firstName}.`} description="Verified Academy evidence about progress, practice, strengths, and next opportunities." environment="student" ariaLabel="Learning evidence" ariaMessage={twin.summary||blankTwin.summary} actions={<>{isStaff&&<button className="button button-primary" onClick={()=>void refreshTwin()}>Refresh evidence</button>}<Link className="button button-secondary" to="/courses">Continue learning</Link></>} stats={[{icon:'✨',value:Number(student.total_xp||0).toLocaleString(),label:'Total XP'},{icon:'🚀',value:`${Math.round(twin.performance_readiness)}%`,label:'Performance ready'},{icon:'💼',value:`${Math.round(twin.career_readiness)}%`,label:'Career ready'}]}><ProgressRing value={twin.creative_health} label="Creative Health" size={132}/></WorkspaceHero>
    {message&&<div className="admin-message">{message}</div>}
    {isStaff&&<div className="journey-selector">{students.map(item=><button key={item.id} className={selected===item.id?'active':''} onClick={()=>setSelected(item.id)}><span>{studentInitials(item)}</span><strong>{studentName(item)}</strong></button>)}</div>}
    <section className="card journey-profile"><div className="journey-identity"><span className="journey-avatar">{studentInitials(student)}</span><div><div className="eyebrow">Artist profile</div><h2>{displayName}</h2><p>{student.email||'JPAC creator'} · {twin.trend} momentum</p></div></div><ProgressRing value={twin.creative_health} label="Creative Health" size={112}/><span className={`journey-risk ${twin.risk_level}`}>{twin.risk_level} support need</span></section>
    ...
    <FacebookTimeline isStaff={isStaff}/>
