...
export function ChooseCareerPathPage() {
  ...
  return <main className="member-launch path-onboarding">
    ...
    <fieldset className="path-options" disabled={busy}><legend>Your creative direction</legend>{careerPaths.map(path => <label className={`path-option ${selected === path.id ? 'selected' : ''}`} key={path.id}>
      <input type="radio" name="career-path" value={path.id} checked={selected === path.id} onChange={() => setSelected(path.id)} />
      <span className="path-symbol" aria-hidden="true">{path.icon}</span><strong>{path.title}</strong><span>{path.outcome}</span><small>{path.connectedPrograms.slice(0, 3).join(' · ')}</small>
    </label>)}</fieldset>
    <footer className="path-confirm"><div><strong>{careerPaths.find(path => path.id === selected)?.title || 'Select one path to continue'}</strong>{error && <p role="alert">{error}</p>}</div><button className="button button-primary" disabled={!selected || busy} onClick={() => void confirm()}>{busy ? 'Saving your path…' : 'Confirm My Career Path'}</button><button className="auth-switch" onClick={() => void signOut()}>Sign out</button></footer>
  </main>;
}
