...
export async function loadMyCourses(){
  ...
  if(moduleError)return{data:courses,error:moduleError.message};
  const scopedCourses=courses.map(course=>({...course,published_module_count:modules.filter(module=>module.course_id===course.course_id).length}));
  if(!modules.length)return{data:scopedCourses,error:''};
  ...
  if(lessonError||!lessons.length)return{data:scopedCourses,error:lessonError?.message||''};
  ...
  const enriched=scopedCourses.map(course=>{
...
export async function loadCourseContent(courseId:string){
  ...
  if(access.error||!access.data)return{course:null,modules:[],lessons:[],progress:[],courseProgress:0,error:access.error?.message||'This course is locked until JPAC staff verifies enrollment and grants access.'};
  const[courseResult,moduleResult]=await Promise.all([supabase.from('courses').select('id,title,slug,description,difficulty,total_xp,wix_program_url').eq('id',courseId).maybeSingle(),supabase.from('course_modules').select('id,course_id,title,description,sort_order,xp_value,level_module_number,short_intro,active_instructional_media_id,primary_video_url,video_title,video_provider,video_duration_seconds,core_xp,bonus_xp_available,jpac_tool_activity,real_world_activity,career_connection,course_level:course_levels(level_number,title)').eq('course_id',courseId).eq('status','published').order('sort_order')]);
