...
export async function loadCourseContent(courseId:string){
  ...
  const[courseResult,moduleResult]=await Promise.all([supabase.from('courses').select('id,title,slug,description,difficulty,total_xp,wix_program_url').eq('id',courseId).maybeSingle(),supabase.from('course_modules').select('id,course_id,title,description,sort_order,xp_value,level_module_number,short_intro,primary_video_url,video_title,video_provider,video_duration_seconds,core_xp,bonus_xp_available,jpac_tool_activity,real_world_activity,career_connection,course_level:course_levels(level_number,title)').eq('course_id',courseId).eq('status','published').order('sort_order')]);
