...
export function runLessonVideoSectionTests(): number {
  ...
  const shortMarkup = renderToStaticMarkup(<LessonVideoSection module={{ ...base, primary_video_url: 'https://youtu.be/abcdefghijk?t=12' }} />);
  const noCookieMarkup = renderToStaticMarkup(<LessonVideoSection module={{ ...base, primary_video_url: 'https://www.youtube-nocookie.com/embed/abcdefghijk' }} />);
  const directMarkup = renderToStaticMarkup(<LessonVideoSection module={{ ...base, primary_video_url: 'https://media.example.com/warm-up.mp4' }} />);
  ...
  assert(shortMarkup.includes('https://www.youtube-nocookie.com/embed/abcdefghijk'), 'Short YouTube URLs must use the safe embed.');
  assert(noCookieMarkup.includes('https://www.youtube-nocookie.com/embed/abcdefghijk'), 'Privacy-enhanced embeds must remain privacy-enhanced.');
  assert(directMarkup.includes('<video') && directMarkup.includes('https://media.example.com/warm-up.mp4'), 'Direct MP4 media must use the native video player.');
