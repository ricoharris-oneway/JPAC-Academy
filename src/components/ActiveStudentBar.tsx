...
export function ActiveStudentBar(){
  ...
  async function load(){
    ...
    const profileRoute=location.pathname.match(/^\/staff\/student\/([0-9a-f-]{36})$/i);
  ...
  return <section className="card active-student-bar" aria-label="Active student workflow" style={{padding:'12px 16px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center',gap:16,flexWrap:'wrap'}}><div style={{display:'grid',gap:2}}><span className="eyebrow">Working with</span><strong>{student?.display_name||email}</strong><small className="muted">{student?.courseTitle||'No active course'}{student?.status?` · ${student.status}`:''}</small></div><nav style={{display:'flex',gap:8,flexWrap:'wrap'}}><Link className="button button-secondary" to={`/staff/course-enrollment?email=${encoded}`}>Enrollment</Link><Link className="button button-secondary" to={`/staff/payment-ledger?email=${encoded}`}>Payment</Link><Link className="button button-secondary" to={`/staff/consent-ledger?email=${encoded}`}>Consent</Link>{student?.id&&<Link className="button button-secondary" to={`/staff/student/${student.id}`}>Profile</Link>}<Link className="button button-secondary" to={`/teacher?email=${encoded}`}>Teacher Studio</Link><button className="button button-secondary" type="button" onClick={clear}>Change student</button></nav></section>;
}
