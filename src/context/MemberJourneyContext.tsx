...
import { careerPathBySlug } from '../data/careerPaths';
...
export function MemberJourneyProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setState({ userId, selectedPath: careerPathBySlug(data?.career_path_slug)?.slug || null, loading: false, error: '' });
    } catch {
  ...
  async function selectPath(slug: string) {
    if (!userId || !supabase || !careerPathBySlug(slug)) throw new Error('Choose a valid Career Path while signed in.');
