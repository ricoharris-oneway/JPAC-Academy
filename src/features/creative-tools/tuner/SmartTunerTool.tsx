...
import { describePitch, detectPitch, practicePrompt, referencePitches, tunerActivities, tunerGoals, tunerHelpers, tuningFeedback, type TunerGoal } from './pitchDetection';
...
export function SmartTunerTool() {
  ...
  const streamRef = useRef<MediaStream | null>(null); const contextRef = useRef<AudioContext | null>(null); const referenceContextRef = useRef<AudioContext | null>(null); const animationRef = useRef<number | null>(null); const historyRef = useRef<number[]>([]); const lastDetectionRef = useRef(0); const mountedRef = useRef(true); const requestIdRef = useRef(0); const sessionStartedRef = useRef(false);

  const stopMic = useCallback(() => {
    requestIdRef.current += 1;
    if (animationRef.current !== null) cancelAnimationFrame(animationRef.current); animationRef.current = null;
    streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null;
    if (contextRef.current) void contextRef.current.close(); contextRef.current = null; historyRef.current = []; setReading(null); setStable(false); setPermission('idle'); if (sessionStartedRef.current) setStoppedAfterPractice(true); setMessage('Microphone stopped. Nothing was recorded or saved.');
  }, []);
  ...
  async function startMic() {
    ...
    try {
      ...
      setPermission('active'); setMicStarted(true); setStoppedAfterPractice(false); sessionStartedRef.current = true; setMessage('Listening locally…');
      const analyze = (time: number) => {
        ...
        if (frequency && frequency >= 55 && frequency <= 1200) {
          ...
          const smoothFrequency = history.reduce((sum, value) => sum + value, 0) / history.length; const spread = history.length > 2 ? Math.max(...history) - Math.min(...history) : Infinity; const isStable = history.length >= 3 && spread / smoothFrequency < .018; const nextReading = describePitch(smoothFrequency); setStable(isStable); setReading(nextReading); setLastReading(nextReading); setNoteObserved(true); setFeedbackObserved(true); if (isStable && Math.abs(nextReading.cents) <= 5) setBestCenteredNote(`${nextReading.note} (${nextReading.cents > 0 ? '+' : ''}${nextReading.cents} cents)`); setMessage('Microphone active. Audio stays in this browser.');
        } else if (time - lastDetectionRef.current > 700) { historyRef.current = []; setReading(null); setStable(false); }
        animationRef.current = requestAnimationFrame(analyze);
      };
      animationRef.current = requestAnimationFrame(analyze);
    } catch (error) {
      streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null; if (!mountedRef.current || requestId !== requestIdRef.current) return; const denied = error instanceof DOMException && (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError'); setPermission(denied ? 'denied' : 'error'); setMessage(denied ? 'Microphone permission was denied. You can update browser permissions and try again.' : error instanceof Error ? error.message : 'The microphone could not start.');
    }
  }
