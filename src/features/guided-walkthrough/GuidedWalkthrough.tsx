...
export function guidedWalkthroughReducer(
  ...
  if (action.type === 'show-onboarding') return { ...state, open: true, view: 'onboarding', stepIndex: 0 };
...
export function GuidedWalkthrough({ initialOpen = false, initialView = 'page', initialPromptVisible = false }: { initialOpen?: boolean; initialView?: GuidedWalkthroughState['view']; initialPromptVisible?: boolean } = {}): JSX.Element {
  ...
  return <div className="guided-walkthrough">
    {promptVisible ? <aside className="aria-onboarding-prompt" aria-label="Aria welcome prompt">
      <GuidedAvatar size="small" />
      <div><strong>Welcome to JPAC Academy. Want me to show you around?</strong><span><button className="button button-primary" type="button" onClick={startOnboarding}>Start JPAC Tour</button><button className="guided-walkthrough-text-button" type="button" onClick={rememberPromptDismissal}>Maybe later</button></span></div>
    </aside> : null}
